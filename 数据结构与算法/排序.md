  ...
  public satic void headSort(int[] array,int size){  //array排序数组，size数组长度
      ...
      for(int i = size-1;i >= 0;i--){
          swap(array,i,0); //将最大的节点与最后一个节点交换，以确保排序数组
         	shiftDown(array,i,0);//默认斩掉最后一个节点
      }
  }
...
```java
//非递归 实现归并排序
   public static void mergetSort1(int[] array){
        int gap = 1;
        ...
        while (gap < array.length){
            for(int i= 0;i < array.length;i+=gap*2){
                ...
                merget1(array,i,mid,right);
            }
            gap*=2;
        }
    }
    private static void merget1(int[] arr,int left,int mid,int right){
        int len = right-left;
